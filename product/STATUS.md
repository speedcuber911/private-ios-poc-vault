...
|---|---|---|
| cloud (full suite, incl. `handoff-ready`, `apns-unconfigured`, `push-banner`) | `cd product/cloud && node --test test/*.test.mjs` | **270 pass / 0 fail** |
| cli (full suite, incl. `handoff-wait` + `syncauth-keychain`) | `cd product/cli && node --test test/*.test.mjs` | **187 pass / 0 fail** |
