...
| Push notification on handoff ready | blocked (not a code gap) | `relayd` → `RELAY_HANDOFF_READY` → cloud fanout | Whole chain verified: the app registers and uploads tokens (25 device rows, all 25 holding APNs tokens), relayd emits the event, the cloud fans out. Blocked only on `APNS_KEY_ID`/`APNS_SIGNING_KEY_P8` |
...
|---|---|---|
| cloud (full suite, incl. `handoff-ready` + `apns-unconfigured`) | `cd product/cloud && node --test test/*.test.mjs` | **250 pass / 0 fail** |
| cli (full suite, incl. `handoff-wait` + `syncauth-keychain`) | `cd product/cli && node --test test/*.test.mjs` | **180 pass / 0 fail** |
...
13. Live credentials + transports: real APNs `.p8` over HTTP/2 (Live Activity channel), SES wiring for magic links, real SIWA service config. `410`/`Unregistered` token cleanup is already implemented and is no longer part of this item. **This is now the single blocker for push notifications** — mint a `.p8` at developer.apple.com → Keys, set `APNS_KEY_ID` and `APNS_SIGNING_KEY_P8` in `/etc/relay-cloud/env`, restart. Match `APNS_HOST` to the build the device token came from, or Apple returns `BadDeviceToken`. Everything upstream is verified working.
